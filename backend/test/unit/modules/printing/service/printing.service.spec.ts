...
describe('PrintingService', () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(service as any, 'generateBarcode')
      .mockResolvedValue('barcode-url');
  });

  // printInvoice
  it('printInvoice throw nếu thiếu orderId', async () => {
    await expect(service.printInvoice('store1', '')).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('printInvoice trả về đúng dữ liệu khi đủ thông tin', async () => {
    mockRepo.findOne
      .mockResolvedValueOnce({
        ...
        total_amount: 100,
        created_at: new Date(),
        customer: { name: 'A', email: 'a@mail.com' },
        payment_method: {},
      }) // order
  ...
  it('printReceipt throw nếu thiếu paymentId', async () => {
    await expect(service.printReceipt('store1', '')).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('printReceipt throw nếu không có payment', async () => {
    ...
    await expect(service.printReceipt('store1', 'pay1')).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it('printBarcode throw nếu thiếu productId', async () => {
    await expect(service.printBarcode('store1', '')).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('printBarcode throw nếu không có product', async () => {
    ...
    await expect(service.printBarcode('store1', 'prod1')).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it('printQuotation throw nếu item thiếu name/quantity/price', async () => {
    ...
    await expect(
      service.printQuotation('store1', {
        ...
        customerName: 'A',
        customerEmail: '',
        items: [{ name: 'A', quantity: 1, price: 0 }],
      }),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it('printQuotation trả về đúng dữ liệu khi hợp lệ', async () => {
    ...
    const result = await service.printQuotation('store1', {
      id: 'q1',
      customerName: 'A',
