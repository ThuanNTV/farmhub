...
describe('AuthService', () => {
  beforeEach(async () => {
    ...
    // Manually inject SecurityService since it's accessed via dynamic property
    (service as any)['securityService'] = securityService;

    // Mock bcrypt.compare to return true for correct password
    jest
      .spyOn(bcrypt, 'compare')
      .mockImplementation((plain: string, hash: string): Promise<boolean> => {
        if (plain === 'password123' && hash) {
...
describe('resetPassword', () => {
  it('should throw UnauthorizedException for invalid token type', async () => {
    ...
    const mockPayload = { user_id: 'user-123', type: 'invalid' };
  ...
  it('should throw UnauthorizedException when reset token does not match', async () => {
    ...
    jwtService.verify.mockReturnValue({ user_id: 'user-123', type: 'reset' });
...
describe('refreshToken', () => {
  ...
  it('should throw UnauthorizedException when user not found', async () => {
    const refreshToken = 'valid.refresh.token';
    const mockPayload = { user_id: 'user-123', type: 'refresh' };
    jwtService.verify.mockReturnValue(mockPayload);
