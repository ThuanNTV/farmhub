...
describe('DispatchOrderItemsController', () => {
  ...
  beforeEach(async () => {
    ...
    controller = module.get<DispatchOrderItemsController>(DispatchOrderItemsController);
  ...
  describe('findByDispatchOrderId', () => {
    it('should return items by dispatch order id', async () => {
      ...
      const result = await controller.findByDispatchOrderId('dispatch-123', mockUser);

      expect(service.findByDispatchOrderId).toHaveBeenCalledWith('dispatch-123', mockUser);
      expect(result).toEqual(mockDispatchOrderItems);
    });
  });
});
