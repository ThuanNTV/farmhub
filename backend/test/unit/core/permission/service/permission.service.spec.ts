...
describe('PermissionService', () => {
  ...
  describe('hasPermission', () => {
    ...
    it('ném lỗi nếu không tìm thấy user', async () => {
      ...
      await expect(service.hasPermission('u1', 'ADMIN')).rejects.toThrow(NotFoundException);
    });
    it('trả về true nếu user là ADMIN_GLOBAL', async () => {
      ...
      await expect(service.hasPermission('u1', 'admin_global')).resolves.toBe(true);
    });
    ...
    it('trả về false nếu user không có quyền', async () => {
      ...
      await expect(service.hasPermission('u1', UserRole.ADMIN_STORE)).resolves.toBe(false);
    });
  });
  ...
  describe('getUserPermissions', () => {
    it('ném lỗi nếu không tìm thấy user', async () => {
      usersRepo.findOne.mockResolvedValue(null);
      await expect(service.getUserPermissions('u1')).rejects.toThrow(NotFoundException);
    });
    it('trả về [*] nếu user là ADMIN_GLOBAL', async () => {
      ...
      await expect(service.getUserPermissions('u1')).resolves.toEqual([UserRole.ADMIN_GLOBAL]);
    });
    it('trả về [role] nếu user có role', async () => {
      ...
      await expect(service.getUserPermissions('u1')).resolves.toEqual([UserRole.ADMIN_STORE]);
    });
  ...
  describe('validateUserStorePermission', () => {
    it('trả về false nếu không có permission', async () => {
      await expect(service.validateUserStorePermission('u1', 's1', '')).resolves.toBe(false);
    });
    it('ném lỗi nếu không có mapping', async () => {
      userStoreMappingsRepo.findOne.mockResolvedValue(null);
      await expect(service.validateUserStorePermission('u1', 's1', 'ADMIN')).rejects.toThrow(ForbiddenException);
    });
    it('trả về true nếu mapping là ADMIN_GLOBAL', async () => {
      ...
      await expect(service.validateUserStorePermission('u1', 's1', UserRole.ADMIN_GLOBAL)).resolves.toBe(true);
    });
    it('trả về true nếu mapping có đúng role', async () => {
      ...
      await expect(service.validateUserStorePermission('u1', 's1', UserRole.ADMIN_STORE)).resolves.toBe(true);
    });
    it('trả về false nếu mapping không có quyền', async () => {
      ...
      await expect(service.validateUserStorePermission('u1', 's1', UserRole.ADMIN_STORE)).resolves.toBe(false);
    });
  });
}); 